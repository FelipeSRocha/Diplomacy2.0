  ...
  switch (respData.resp) {
    ...
    case "UpdateInfluency":
      ...
      Object.keys(DB.players).forEach((id) => {
        ...
        const influenciedcountry = DB.players[id].territories.includes(
          respData.other
        );
    ...
    case "NextRound":
      ...
      if ((Quarter == 1 && DB.Round > 1) || (Quarter == 3 && DB.Round == 2)) {
        ...
        VIEW.DisplayEvent(
          DB.Events[numberEvent].nome,
          DB.Events[numberEvent].efeito
        );
      } else if (Quarter == 3) {
        const numberEvent = DB.Events.activeEvents[1][0];
        VIEW.DisplaySecondEvent(
          DB.Events[numberEvent].nome,
          DB.Events[numberEvent].efeito
        );
      }
