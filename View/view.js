class View {
  ...
  RenderTitle() {
    ...
    function createHelp() {
        Inhtml.AddBtn("diceTitleDiv", "helpBtn", "helpBtn", "Rules", () => {
          alternateHelp();
        });
        Inhtml.AddElement("div", "body", "helpBox", "helpBox-hide");
        Inhtml.AddElement("h1", "helpBox", "", "", "How to Play");
        Inhtml.AddElement("h2", "helpBox", "", "", "Preparation");
        Inhtml.AddElement(
          "p",
          "helpBox",
          "",
          "",
          "Choose if the game is clockwise or counterclockwise and the first player, each round, the first player advances in the chosen direction."
        );
        Inhtml.AddElement(
          "p",
          "helpBox",
          "",
          "",
          "Each player rolls 2 supply dice and takes 2 of each resource according to the result. Realize Acquire a territory action."
        );
      
        Inhtml.AddElement("h2", "helpBox", "", "", "Objective");
        Inhtml.AddElement(
          "p",
          "helpBox",
          "",
          "",
          "The objective of the game is to fulfill the 3 objectives, players can influence any decision of any other player as long as in the due game turn offering any game actions."
        );
        Inhtml.AddElement("h2", "helpBox", "", "", "Rounds");
        Inhtml.AddElement(
          "p",
          "helpBox",
          "",
          "",
          "At the beginning of each round, countries will produce the indicated tokens for all players who have their influence, after this, each player will be able to do any actions as they wish."
        );
        Inhtml.AddElement("h2", "helpBox", "", "", "Actions");
        Inhtml.AddElement(
          "p",
          "helpBox",
          "",
          "",
          "• Acquire a territory: Pay a adjacent territory to yours twice as many tokens next to the his name, claim the territory influency."
        );
        Inhtml.AddElement(
          "p",
          "helpBox",
          "",
          "",
          "• Passage: Pay the number of resources that the territory produces for each territory you want to cross, if the territory is neutral, pay the resource that is not indicated next to the territory's name, if it is a player's territory, the resources are of his choice."
        );
        Inhtml.AddElement(
          "p",
          "helpBox",
          "",
          "",
          "• Establish Allied Territories: Negotiate with the territory's owner so that this territory is considered yours as well, producing the same amount for both players, claiming the territory's influence as well."
        );
        Inhtml.AddElement(
          "p",
          "helpBox",
          "",
          "",
          "• Exchange resources: Exchange 4 of the same resource with the bank for another resource of your choice, trading with other players is free."
        );
        Inhtml.AddElement("h2", "helpBox", "", "", "Notes");
        Inhtml.AddElement(
          "p",
          "helpBox",
          "",
          "",
          "• Every first (except the very first one) and third round there is a convention event, some are automated"
        );
        Inhtml.AddElement(
          "p",
          "helpBox",
          "",
          "",
          "• Only two players can influency the same country (not automated)"
        );
        Inhtml.AddElement(
            "p",
            "helpBox",
            "",
            "",
            "• no exchanges or purchases are automated"
          );
        Inhtml.AddBtn("helpBox", "helpBtn", "helpBtn", "Exit", () => {
            alternateHelp();
          });
      }
      function alternateHelp() {
        const helpbox = document.getElementById("helpBox");
        if (helpbox.className == "helpBox-hide") {
          helpbox.className = "helpBox";
        } else {
          helpbox.className = "helpBox-hide";
        }
      }
      createHelp();
  }
    ...
    Object.keys(DB.players).forEach((id) => {
      ...
      if (pN < 2) {
        ...
        if (id == myClientId) {
          ...
          this.Inhtml.AddElement(
            ...
            `IDobjinfluency_${pN}`,
            `Classobj`,
            DB.players[id].objective[2][1]
          );
        }
        this.Inhtml.AddElement(
          "div",
          `IDPlayerDiv_${pN}`,
          `IDPlayerDivTitle_${pN}`,
          `ClassPlayerDivTitle`
        );
        this.Inhtml.AddElement(
          "h1",
      ...
      Object.keys(DB.players[id].prod).forEach((key) => {
        this.Inhtml.AddElement(
          "div",
          `IDPlayerProd_${pN}`,
          `IDPlayerProd_${key}_${pN}`,
          `ClassPlayerProd_each`
        );
        ...
        this.Inhtml.AddElement(
          ...
          "-1",
          () => {
            sendAction("ModifyValue", [id, "prod", key, -1]);
          }
        );
        this.Inhtml.AddElement(
          "button",
        ...
        this.Inhtml.AddElement(
          ...
          "-5",
          () => {
            sendAction("ModifyValue", [id, "prod", key, -5]);
          }
        );
        this.Inhtml.AddElement(
          "button",
      ...
      Object.keys(DB.players[id].bank).forEach((key) => {
        this.Inhtml.AddElement(
          "div",
          `IDPlayerBank_${pN}`,
          `IDPlayerBank_${key}_${pN}`,
          `ClassPlayerBank_each`
        );
        ...
        this.Inhtml.AddElement(
          ...
          "-1",
          () => {
            sendAction("ModifyValue", [id, "bank", key, -1]);
          }
        );
        this.Inhtml.AddElement(
          "button",
        ...
        this.Inhtml.AddElement(
          ...
          "-5",
          () => {
            sendAction("ModifyValue", [id, "bank", key, -5]);
          }
        );
        this.Inhtml.AddElement(
          "button",
  ...
  RenderInfoCountry(NameofCountry) {
    ...
    this.Inhtml.AddBtn(
      ...
      `Claim Influency`,
      () => {
        this.getInfluencyInfo(NameofCountry);
      }
    );
  }
  RenderFooter() {
  ...
  FindDimensionsofElement(IDElement) {
    ...
    const dimensions = {
      ...
      width: w_place,
      height: h_place,
      x_center: center_x_place,
      y_center: center_y_place,
    };
    return dimensions;
  }
  ColorPlayerCircle(country, pN, id, color) {
    let circle = document.getElementById(`${country}_2`).children[2].children[
      pN
    ];
    if (color) {
